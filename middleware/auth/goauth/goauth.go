...
func GenerateJWT(id string, data map[string]any) string {
	claim := jwt.MapClaims{
		"id": id,
	}
	if len(data) > 0 {
...
func (g *GoAuth) getTokenFromRedis(ctx context.Context, clientId string) (any, error) {
...
func (g *GoAuth) getUserFromRedis(ctx context.Context, clientId string) (any, error) {
...
func (g *GoAuth) insertClientToRedis(ctx context.Context, client AuthClient) error {
	...
	clientStoreInfo := map[string]any{clientId: jsonClientString}
	tokenStoreInfo := map[string]any{token: jsonClientString}
